...
(function() {
  ...
  module.exports = {
    ...
    disconnectFromAP: function(_ap) {
      var COMMANDS, _msg, com, disconnectFromAPChain, error, i, len, stdout;
      COMMANDS = {
        disconnect: "networksetup -removepreferredwirelessnetwork " + this.WiFiControlSettings.iface + " \"" + _ap.ssid + "\""
      };
      disconnectFromAPChain = ["connect"];
      for (i = 0, len = disconnectFromAPChain.length; i < len; i++) {
        com = disconnectFromAPChain[i];
        this.WiFiLog("Executing:\t" + COMMANDS[com]);
        try {
          stdout = this.execSync(COMMANDS[com]);
        } catch (error1) {
